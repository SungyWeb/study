...
3. 字符串是一个类数组对象，可以被解构，而且可以将length属性解构
...
5. 由于null和undefined无法转成对象，所以无法解构，null和undefined上没有任何属性  
...
2. 串模板字符如果紧跟在函数名后面，该函数将被调用来处理这个模板字符串   
...
3. 新方法 includes startWith endWith

## 数值

1. Math.trunc 去除小数部分，返回整数，参数不是数值先转成数值，对于空值和无法取整的返回NaN
...
2. 箭头函数
      ...
      + this指向，不能通过bind改变this
      + 不能用于原型的构造函数即不能new一个箭头函数
      + 不能用arguments对象，可以用`(...rest)`
      + 不可以使用yield命令，因此箭头函数不能用作 Generator 函数

## 数组

1. 将字符串转换成数组可以split，还可以
...
2. Array.of 将一组值转化为数组,参数为空，返回空数组
...
4. forEach遍历数组时，不可以退出循环，可以用every或some代替
...
3. Object.assign() 只拷贝对象的自身属性（包括symbol属性，但不拷贝继承属性和不可枚举属性），注意点
...
13. class
       ...
       + 定义： ES6 的类，完全可以看作构造函数的另一种写法，类的数据类型就是函数，类本身就指向构造函数
       ...
       + constructor 是默认方法，如果没有显式定义，一个空得constructor 会被默认添加
    
       + getter/setter 对某个属性设置取值函数和存值函数，拦截该属性的存取行为
         ...
         ```js
          ...
          // getter setter 会在属性的description上体现
          const desc = Object.getOwnPropertyDescriptor(MyClass.prototype, "prop");
          console.log(desc);
         ```
