...
- **基础版备忘录** ([`/README.md`](/README.md#basic-cheatsheet-table-of-contents))专注于React开发者在React应用中如何开始使用TS进行开发。
  - 专注于可移植的最佳实践
  - 解释了一些基本的TS用法和设置方法
  - 回答那些经常被问到的问题
  - 不详细介绍泛型类型逻辑。相反，我们更喜欢教新手简单的故障排除技巧。
  - 目标的是不需要学习太多的TS知识就能够理解和使用。
- **高级版备忘录** ([`/ADVANCED.md`](/ADVANCED.md)) 展示了开发者在编写可复用的程序/函数/渲染属性/高阶组件和TS+React库时泛型的一些高级用法。
  - 为高级开发者提供了各种各样的技巧和提示。
  - 在定义类型时提供一些建议。
  - 目标是充分利用TS。
- **The Migrating Cheatsheet** ([`/MIGRATING.md`](/MIGRATING.md)) 提供了一些从js和flow的大型项目做增量迁移时的建议。
  - 我们不试图去劝说别人，只是想帮助那些已经确定的人。
  - ⚠️这是一个新的备忘录，欢迎提出意见。
- **The HOC Cheatsheet** ([`/HOC.md`](/HOC.md)) 通过实例来教会大家如何编写高阶组件（HOC）。
  - 熟悉 [Generics泛型](https://www.typescriptlang.org/docs/handbook/generics.html)是有必要的。
  - ⚠️这是最新的备忘录，欢迎提出意见。
...
- [Section 1: 安装](#section-1-setup)
  - [预备](#prerequisites)
  - [React + TypeScript 开始工具](#react--typescript-starter-kits)
  - [Import React](#import-react)
- [Section 2: 开始](#section-2-getting-started)
...
1. 很好的理解[React](https://reactjs.org)
2. 熟悉[TypeScript 基础类型](https://www.typescriptlang.org/docs/handbook/basic-types.html) (可以参考[2ality's guide](http://2ality.com/2018/04/type-notation-typescript.html))
3. 已经读过[React官方文档和TS部分](https://reactjs.org/docs/static-type-checking.html#typescript).
4. having read [新版 TypeScript playground中React部分](http://www.typescriptlang.org/play/index.html?jsx=2&esModuleInterop=true&e=181#example/typescript-with-react) (可选: 也可以参考[the playground's](http://www.typescriptlang.org/play/index.html)上面的40+的实例)

本指南将始终假设您从最新的TypeScript版本开始。旧版本的注释将位于可展开的“详细信息”标记中。

## React + TypeScript 开始工具
...
- 我们曾建议`create-react-app-typescript`，但现在已[弃用]](https://www.reddit.com/r/reactjs/comments/a5919a/createreactapptypescript_has_been_archived_rip/)。 [请参阅迁移说明](https://vincenttunru.com/migrate-create-react-app-typescript-to-create-react-app/)。

2. [Basarat's guide](https://github.com/basarat/typescript-react/tree/master/01%20bootstrap) 手动设置 React + TypeScript + Webpack + Babel

- 需要指出的是，要确保你已经安装了 `@types/react` 和 `@types/react-dom` ([如果你不熟悉类型声明项目，点击查看更多信息](https://definitelytyped.org/))
- 这里也有很多 React + TypeScript 模板, 请查看 [下面的清单](https://github.com/typescript-cheatsheets/react-typescript-cheatsheet#recommended-react--typescript-codebases-to-learn-from).
...
在 [TypeScript 2.7+](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-2-7.html)的普通jsx中, 你可以通过 `--allowSyntheticDefaultImports` 运行TS(或者在tsconfig中添加`"allowSyntheticDefaultImports": true` ) 来引入:
...
为什么用 `allowSyntheticDefaultImports` 而不是 `esModuleInterop`? [Daniel Rosenwasser](https://twitter.com/drosenwasser/status/1003097042653073408) 已经说过了，这样对webpack/parcel更加友好. 更多详情点击 <https://github.com/wmonk/create-react-app-typescript/issues/214>

你也应该查看新版的TS官方文档中关于每一个编译指令 [新版的TS官方文档中关于每一个编译指令](https://www.typescriptlang.org/v2/en/tsconfig#allowSyntheticDefaultImports)之间的差异!
...
它们可以可以像普通函数一些书写，需要`props`参数并且返回一个JSX element。
...
<summary><b>什么是`React.FC`/`React.FunctionComponent`?</b></summary>

你也可以使用 `React.FunctionComponent` (简写 `React.FC`):
...
- 它提供了对于静态属性的类型检查和自动完成，比如 `displayName`, `propTypes`, and `defaultProps` - **但是**，在`React.FunctionComponent`中使用 `defaultProps` 已经发现了一些问题. 查看 [更多信息](https://github.com/typescript-cheatsheets/react-typescript-cheatsheet/issues/87) - 我们的 `defaultProps` 部分有些建议.
